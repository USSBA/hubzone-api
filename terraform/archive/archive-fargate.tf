...
locals {
  archive_container_environment = {
    ...
    HUBZONE_API_DB_HOST  ="archive-hubzone-db.${local.env.fqdn_base}"
  }
  archive_container_secrets_parameterstore = {
  ...
  container_definitions = [
    {
      ...
      environment = [for k, v in local.archive_container_environment : { name = k, value = v }]
      secrets     = [for k, v in local.archive_container_secrets_parameterstore : { name = k, valueFrom = "${local.prefix_parameter_store}/${v}" }]
    },
  ]
}
